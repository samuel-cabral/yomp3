    ...
    private func dispatch(items: [DownloadItem]) {
        for item in items {
            ...
            let task = Task { @MainActor [weak self] in
                ...
                self.queue.update(id) { $0.status = .downloading(progress: 0) }
                do {
                    let outputDir = Preferences.outputDirectory
                    for try await event in self.runner.download(sourceURL, into: outputDir) {
                        switch event {
                        case .progress(let p): self.queue.update(id) { $0.status = .downloading(progress: p) }
                        case .done(let fileURL): self.queue.update(id) { $0.status = .done(fileURL) }
                        case .filename: break
                        }
                    }
                } catch {
                    self.queue.update(id) { $0.status = .failed(error.localizedDescription) }
                }
